...
const FoodList = () => {
    ...
    const [foodFull, setFoodFull] = useState([])
    ...
    const fetchFood = async () => {
        try {
            ...
            const food = await getAllFood(currentPage, dataPerPage, searchTerm, "");
            setFood(food.data);
            setTotalPages(Math.ceil(food.data.totalItems / dataPerPage));
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const fetchFoodFull = async () => {
        try {
            const foodFull = await getAllFood("", 1000, "");
            setFoodFull(foodFull.data);
            console.log("LOLLL>", foodFull.data);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const exportToExcel = () => {
        const columnsToExport = ['name', 'calories', 'serving_size', 'serving_plate', 'serving_bowl', 'piece', 'fat', 'cholesterol', 'carbohydrate', 'protein', 'sodium', 'kalium'];
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'FoodData');
        XLSX.writeFile(workbook, 'List Makanan.xlsx');
    };
            ...
            <table className="bg-white border-2 border-gray-300 w-full text-center 2xl:rounded-lg">
                <thead className="px-2">
                    <tr >
                        ...
                        <th className="py-1 md:py-2 lg:py-2 2xl:py-2 text-xs md:text-md lg:text-md 2xl:text-xl">Takaran</th>
                ...
                <tbody >
                    {food.map((item) => (
                        <tr key={item.id}>
                            ...
                            <td className="px-2 md:py-2 lg:py-2 2xl:py-2 text-xs md:text-md lg:text-md 2xl:text-xl">{item.serving_size}</td>
                            <td className="px-2 md:py-2 lg:py-2 2xl:py-2 text-xs md:text-md lg:text-md 2xl:text-xl">{item.calories}</td>
