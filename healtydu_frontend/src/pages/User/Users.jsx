...
import { getAllUser } from "../../fetching/user"
...
const User = () => {
    ...
    const fetchUser = async () => {
        try {
            const user = await getAllUser(currentPage, dataPerPage, searchTerm, "");
            setUser(user.data);
            setTotalPages(Math.ceil(user.data.totalItems / dataPerPage));
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const fetchUserFull = async () => {
        try {
            const userFull = await getAllUser("", 1000, "", "");
            ...
            <table className="bg-white border-2 border-gray-300 w-full text-center 2xl:rounded-lg">
                <thead className="px-2">
                    <tr >
                        ...
                        <th className="py-1 md:py-2 lg:py-2 2xl:py-2 text-xs md:text-md lg:text-md 2xl:text-xl">Email</th>
                ...
                <tbody >
                    {user.map((item) => (
                        <tr key={item.id}>
                            ...
                            <td className="px-2 md:py-2 lg:py-2 2xl:py-2 text-xs md:text-md lg:text-md 2xl:text-xl">{item.email}</td>
