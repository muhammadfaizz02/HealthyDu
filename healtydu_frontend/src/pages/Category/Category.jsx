...
const Category = () => {
    ...
    const fetchCategory = async () => {
        try {
            ...
            const categroy = await getAllCategory(currentPage, dataPerPage, searchTerm, "");
            setCategory(categroy.data);
            setTotalPages(Math.ceil(categroy.data.totalItems / dataPerPage));
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const fetchCategoryFull = async () => {
        try {
            const categoryFull = await getAllCategory("", 1000, "", "");
            setCategoryFull(categoryFull.data);
            console.log("LOLLL>", categoryFull.data);
        } catch (error) {
            console.log(error);
        }
    };
