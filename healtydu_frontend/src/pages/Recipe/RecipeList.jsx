...
const RecipeList = () => {
    ...
    const fetchCategory = async () => {
        try {
            ...
            setTotalPages(Math.ceil(recipe.data.totalItems / dataPerPage));
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const fetchCategoryFull = async () => {
        try {
            const recipeFull = await getAllRecipe("", 1000, "", "");
            setRecipeFull(recipeFull.data);
            console.log("LOLLL>", recipeFull.data);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Resep Data');
        XLSX.writeFile(workbook, 'List Resep.xlsx');
    };
        ...
        <div className="m-4">
            ...
            <table className="bg-white border-2 border-gray-300 w-full text-center 2xl:rounded-lg">
                <thead className="px-2">
                    <tr >
                        <th className="py-1 md:py-2 lg:py-2 2xl:py-2 text-xs md:text-md lg:text-md 2xl:text-xl">Gambar</th>
                ...
                <tbody >
                    {recipe.map((item) => (
                        <tr key={item.id}>
                            <td className="px-2 md:py-2 lg:py-2 2xl:py-2 text-xs md:text-md lg:text-md 2xl:text-xl"><img src={item.image_url} alt="" className="rounded-md" /></td>
