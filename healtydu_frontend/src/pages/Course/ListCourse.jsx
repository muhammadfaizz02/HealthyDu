...
const ListCourse = () => {
    ...
    const fetchCategory = async () => {
        try {
            const course = await getAllCourse(currentPage, dataPerPage, searchTerm, "");
            setCourse(course.data);
            setTotalPages(Math.ceil(course.data.totalItems / dataPerPage));
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const fetchCategoryFull = async () => {
        try {
            const courseFull = await getAllCourse("", 1000, "", "");
            setCourseFull(courseFull.data);
            console.log("LOLLL>", courseFull.data);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(workbook, 'list_course.xlsx');
    };
        ...
        <div className="m-4">
            <div className="flex justify-between items-center pb-4 2xl:pb-6">
                ...
                <div className="justify-center items-center gap-2 hidden md:flex lg:flex 2xl:flex">
                    ...
                    <Button onClick={handleClick} text="Tambah Course" border="border-sky-700" bg="bg-sky-500" width="w-1/2 md:w-2/6 lg:w-1/4 2xl:w-2/6" />
                </div>
                <div className="flex flex-col md:hidden lg:hidden 2xl:hidden mx-1">
                    <Button onClick={handleClick} text="Tambah Course" border="border-sky-700" bg="bg-sky-500" width="px-3" />
                </div>
